...
# Add Master Hub to path for shared config/credentials
...
# claudeagent directory — used to resolve local files (jp_kb.db, rulebook, etc.)
_CLAUDEAGENT_DIR = Path(__file__).parent
...
class JPChannelAgentTrained:
    ...
    def __init__(self, claude_api_key=None, kb_db=None, instruments_csv=None, rulebook_path=None):
        self.logger = logging.getLogger('JP_TRAINED')

        # Resolve credentials and paths from MasterConfiguration if not provided
        from master_resource import get_claude_key, get_instruments_path
        if claude_api_key is None:
            claude_api_key = get_claude_key()
        if instruments_csv is None:
            instruments_csv = get_instruments_path()
        if kb_db is None:
            kb_db = str(_CLAUDEAGENT_DIR / 'jp_kb.db')
        if rulebook_path is None:
            rulebook_path = str(_CLAUDEAGENT_DIR / 'jp_channel_rulebook.txt')
  ...
  "action": "BUY"
        ...
        try:
            ...
            if json_match:
                ...
                # Add default stop_loss if not provided (15% buffer for options)
                if 'stop_loss' not in data or data.get('stop_loss') in [None, 0, 0.0, '']:
                    entry_price = float(data['entry_price'])
                    option_type = data['option_type']
                    
                    # CE: SL = entry * 0.85 (15% below entry)
                    # PE: SL = entry * 1.15 (15% above entry)
                    if option_type == 'CE':
                        data['stop_loss'] = round(entry_price * 0.85, 2)
                    else:  # PE
                        data['stop_loss'] = round(entry_price * 1.15, 2)
                    
                    self.logger.info(f"[DEFAULT SL] No stop_loss provided, using 15% buffer: {data['stop_loss']}")
                
                # Ensure stop_loss is a number
                data['stop_loss'] = float(data.get('stop_loss', 0))
                ...
                # Build tradingsymbol using correct format
                # Handles weekly vs monthly for NIFTY/SENSEX automatically
                data['tradingsymbol'] = get_correct_tradingsymbol(
                    symbol=data['symbol'],
                    strike=data['strike'],
                    option_type=data['option_type'],
...
def test_trained_agent():
    ...
    from master_resource import get_claude_key
    api_key = get_claude_key()
    if not api_key:
        print("Error: Claude API key not found in MasterConfiguration")
        sys.exit(1)

    # Initialize (paths resolved automatically via MasterConfiguration)
