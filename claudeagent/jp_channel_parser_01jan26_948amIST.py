...
class JPChannelParser:
    ...
    def _get_lot_size(self, symbol, strike, option_type):
        ...
        defaults = {
            ...
            'BANKNIFTY': 35,  # Error said: "should be multiple of 35"
            'SENSEX': 10,
            'BANKEX': 10,
            'FINNIFTY': 40,
            'MIDCPNIFTY': 75
    ...
    def _enrich(self, result, message_date=None):
        """Enrich result with expiry and tradingsymbol using message date"""
        ...
        if message_date is None:
            ...
            # Parse ISO format and make timezone-naive for comparison
        ...
        # Determine expiry based on message date
        if symbol in self.INDEX_SYMBOLS:
            # Weekly expiry for indices
            expiry_date = self._get_next_weekly_expiry(symbol, reference_date)
        else:
            # Monthly expiry for stocks (last Thursday of message's month)
            expiry_date = self._get_last_thursday_of_month(reference_date.year, reference_date.month)
            
            # If that Thursday already passed in the message date, use next month
            expiry_dt = datetime.strptime(expiry_date, '%Y-%m-%d')
            if expiry_dt < reference_date:
                ...
                expiry_date = self._get_last_thursday_of_month(next_year, next_month)
        ...
        if symbol in self.INDEX_SYMBOLS:
            ...
            # Stock format: TATASTEEL26JAN25180CE (includes day "25")
        ...
        if symbol in ['SENSEX', 'BANKEX']:
            ...
            result['exchange'] = 'NSE'  # Stock options
