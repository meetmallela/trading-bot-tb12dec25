...
# Import tradingsymbol utility for correct format generation
...
class JPChannelParser:
    ...
    def _load_instruments_cached(self, csv_path):
        ...
        # Always store cache next to this file so it works regardless of cwd
        cache_file = str(Path(__file__).parent / 'jp_instruments_cache.pkl')
    ...
    def _enrich(self, result, message_date=None):
        ...
        # Build tradingsymbol as fallback using correct format
        # Uses weekly format for NIFTY/SENSEX, monthly for others
        result['tradingsymbol'] = get_correct_tradingsymbol(
            symbol=symbol,
            strike=strike,
            option_type=option_type,
