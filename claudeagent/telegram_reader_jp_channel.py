"""
telegram_reader_jp_channel.py
Telegram reader for JP Paper trade channel -1003282204738
Uses specialized JP parser for index + stock options
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('telegram_jp.log', encoding='utf-8'),
...
logger = logging.getLogger('TELEGRAM_JP')
...
JP_CHANNEL_ID = -1003282204738  # JP Paper trade channel

# Initialize client
client = TelegramClient('jp_channel_bot', API_ID, API_HASH)

# Initialize database
db = sqlite3.connect('jp_signals.db', check_same_thread=False)
db.execute('''
...
# Initialize parser
try:
    # Try with Claude API if available
    ...
    logger.info("[OK] Parser initialized with Claude API")
except:
    # Fallback to regex-only
    parser = JPChannelParser(instruments_csv='valid_instruments.csv')
    logger.info("[OK] Parser initialized (regex-only, no Claude)")
...
def insert_signal(channel_id, channel_name, message_id, raw_text, parsed_data):
    ...
    try:
        cursor = db.cursor()
        signal_type = parsed_data.get('message_type', 'unknown')
        ...
        if cursor.rowcount > 0:
            ...
            logger.info(f"[STORED] Signal ID: {signal_id} ({signal_type})")
            return signal_id
        else:
...
async def handle_message(event):
    ...
    try:
        ...
        if parsed_data:
            ...
            stats['skipped'] += 1
            logger.info(f"[SKIP] Not a trading signal or update message")
...
async def main():
    """Main function"""
    print("\n" + "="*70)
    print("JP CHANNEL TELEGRAM READER")
    print("Channel: JP Paper trade - Dec-25")
    print("="*70)
    print()
    ...
    try:
        ...
        logger.info(f"     Parser: JP Custom (Index + Stock options)")
    except Exception as e:
    ...
    logger.info("  - Index options (NIFTY, BANKNIFTY, SENSEX)")
    logger.info("  - Stock options (all stocks)")
    logger.info("  - Auto stop-loss generation")
    logger.info("  - Smart skip (updates, commentary)")
    logger.info("")
