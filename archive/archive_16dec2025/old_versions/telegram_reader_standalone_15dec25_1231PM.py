...
def resolve_mcx_monthly_expiry(symbol, month_text):
    ...
    # If expiry month already passed, move to next year
    ...
    # Adjust to last working day (Mon–Fri)
...
class TelegramSignalReader:
    ...
    async def process_message(self, event):
        ...
        # ===== RESOLVE MONTH-BASED MCX EXPIRY (DECEMBER, JAN, etc.) =====
# ===== RESOLVE MCX EXPIRY (MONTH OR DEFAULT) =====
        if parsed_signal and isinstance(parsed_signal, dict):
            symbol = parsed_signal.get('symbol')
            expiry_text = parsed_signal.get('expiry_text')  # DECEMBER, JAN, etc.

            mcx_symbols = ['GOLD', 'GOLDM', 'SILVER', 'SILVERM', 'CRUDEOIL', 'NATURALGAS']

            # 1️⃣ Month-based expiry (highest priority)
            ...
            # 2️⃣ Fallback ONLY if expiry still missing
            if symbol in ['NATURALGAS', 'CRUDEOIL'] and not parsed_signal.get('expiry_date'):
                ...
                if fallback_expiry:
                    parsed_signal['expiry_date'] = fallback_expiry
                    logger.info(f"[EXPIRY FALLBACK] {symbol} → {fallback_expiry}")
