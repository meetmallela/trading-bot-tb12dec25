...
def find_instrument_csv(symbol, strike, option_type, expiry_date=None):
    """Quick function to find from CSV"""
    try:
        with open('instruments_cache.csv', 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                if (symbol.upper() in row.get('symbol', '').upper() and
                    row.get('strike') == strike_str and
                    row.get('option_type') == option_type.upper()):
                    
                    if expiry_date:
                        if expiry_date not in row.get('expiry_date', ''):
                            continue
