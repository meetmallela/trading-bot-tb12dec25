    ...
    def calculate_trailing_sl(self, position, current_ltp, current_sl_trigger, signal_data=None):
        ...
        if quantity > 0:
            ...
            if signal_data and signal_data.get('stop_loss'):
                # Signal-based: use original SL (no trailing for now)
                new_sl = signal_data.get('stop_loss')
                return new_sl, "SIGNAL"
            else:
                # Manual order: RECURSIVE TRAILING LOGIC
                ...
                # Calculate threshold: 105% of CURRENT SL (not buy price!)
                threshold_price = current_sl_trigger * 1.05
                
                if current_ltp >= threshold_price:
                    # Trail SL up by moving it to 105% of CURRENT SL
                    # This ensures 5% buffer from current price
            ...
            for pos in open_positions:
                ...
                if existing_sl:
                    ...
                    if sl_type == "TRAILING" and new_sl > current_sl_trigger:
                        ...
                        if sl_type == "HOLD":
                            threshold = current_sl_trigger * 1.05
                            logging.info(f"   [OK] SL at {current_sl_trigger} | Need LTP >= {threshold:.2f} to trail")
                        else:
