...
class EnhancedSLMonitor:
    ...
    def calculate_trailing_sl(self, position, current_ltp, signal_data=None):
        """
        Calculate trailing SL based on position type:
        
        MANUAL ORDERS (no signal):
        - If price moved 5% above buy: Trail SL to 104% of buy price
        - Buy @ 150, LTP @ 157.5 (5%+) -> Trail SL to 156 (104% of 150, rounded down)
        
        SIGNAL-BASED ORDERS:
        - Use original signal's stop_loss
        - Can add custom trailing logic here if needed
        """
        avg_price = abs(position['average_price'])
        ...
        if quantity > 0:
            ...
            if signal_data and signal_data.get('stop_loss'):
                # Signal-based: use original SL (can add trailing logic later)
                new_sl = signal_data.get('stop_loss')
                return new_sl, "SIGNAL"
            else:
                # Manual order: Check if price moved 5% above buy
                threshold_price = avg_price * 1.05  # 5% above buy
                
                if current_ltp >= threshold_price:
                    # Trail SL to 104% of buy price
                    new_sl = int(avg_price * 1.04)  # Round down
                    return new_sl, "TRAILING"
                else:
                    # Not yet 5% profit, keep original SL
                    new_sl = int(avg_price * 0.95)  # 95% of buy
                    return new_sl, "INITIAL"
        
        # For SHORT positions (quantity < 0) - not implemented yet
        else:
            return int(avg_price * 1.05), "INITIAL"
        ...
        try:
            ...
            for pos in open_positions:
                ...
                if existing_sl:
                    ...
                    new_sl, sl_type = self.calculate_trailing_sl(pos, ltp, signal_data)
                    
                    if sl_type == "TRAILING" and new_sl > current_sl_trigger:
                        # Trail the SL upward
                        logging.info(f"   [TRAILING] LTP: {ltp:.2f} >= 5% threshold")
                        ...
                        logging.info(f"   [OK] SL exists at {current_sl_trigger} | Type: {sl_type}")
