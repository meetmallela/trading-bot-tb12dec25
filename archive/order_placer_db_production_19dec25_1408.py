...
class OrderPlacerProduction:
    ...
    def is_too_close_to_expiry(self, expiry_date_str):
        expiry_date = datetime.strptime(expiry_date_str, '%Y-%m-%d').date()
        days_to_expiry = (expiry_date - datetime.now().date()).days
        # Block fresh entries if 0 or 1 day left
    ...
    def process_pending_signals(self):
        ...
        for signal in signals:
            try:
                ...
                # --- FIXED: Expiry check moved inside the loop where 'signal_data' is defined ---
                if self.is_too_close_to_expiry(signal_data.get('expiry_date')):
                    logging.warning(f"[REJECT] {signal_data.get('symbol')} is too close to expiry ({signal_data.get('expiry_date')})")
...
import requests # Added to catch ConnectionErrors

def initialize_kite_with_retry(config):
    """
    Attempt to connect to Kite API with infinite retry on network/DNS failure.
    Ensures the bot stays alive during internet flickers.
    """
    attempt = 0
    while True:
        attempt += 1
        try:
            logging.info(f"[CONNECT] Attempt #{attempt} to initialize Kite...")
            kite = KiteConnect(api_key=config['api_key'])
            kite.set_access_token(config['access_token'])
            
            # This call verifies if the internet is actually working
            profile = kite.profile()
            logging.info(f"[✓ OK] Connected as {profile['user_name']}")
            return kite
            
        except (requests.exceptions.ConnectionError, requests.exceptions.Timeout) as e:
            # Catch DNS errors (Errno 11001) or network drops
            logging.error(f"[X] Internet/DNS Error: {e}")
            logging.info("[RETRY] Waiting 10 seconds before next attempt...")
            time.sleep(10)
        except Exception as e:
            # Catch other issues like invalid access_token
            logging.error(f"[FATAL] Critical Kite Init Error: {e}")
            return None

def main():
    parser = argparse.ArgumentParser(description='Production Order Placer')
    parser.add_argument('--continuous', action='store_true', help='Run in loop')
    ...
    try:
        ...
        logging.error(f"[ERROR] Could not load kite_config.json: {e}")
        return

    # Initialize KiteConnect with Auto-Retry logic
    kite = None
    if not args.test:
        kite = initialize_kite_with_retry(config)
        if not kite:
            logging.error("[ERROR] Failed to initialize Kite after critical error. Exiting.")
            return
    
    # Initialize order placer with the kite object (or None if in test mode)
    ...
    if args.continuous:
        ...
        try:
            while True:
                ...
                # Wrap process_pending_signals in a try-except to keep the loop running
                try:
                    placer.process_pending_signals()
                except (requests.exceptions.ConnectionError, requests.exceptions.Timeout):
                    logging.warning("[CONN] Internet lost during processing. Skipping this cycle.")
                except Exception as e:
