...
class SignalParserWithFutures:
    ...
    def _parse_futures(self, message):
        ...
        # Action - Check for explicit BUY/SELL or infer from ABOVE/BELOW
        action_match = re.search(r'\b(BUY|SELL)\b', message, re.IGNORECASE)
        if action_match:
            result['action'] = action_match.group(1).upper()
        else:
            # Infer action from ABOVE/BELOW keywords
            if re.search(r'\bABOVE\b', message, re.IGNORECASE):
                result['action'] = 'BUY'
                self.logger.info(f"[ACTION] Inferred BUY from 'ABOVE' keyword")
            elif re.search(r'\bBELOW\b', message, re.IGNORECASE):
                result['action'] = 'SELL'
                self.logger.info(f"[ACTION] Inferred SELL from 'BELOW' keyword")
            else:
                # Default to BUY if no action found
                result['action'] = 'BUY'
                self.logger.info(f"[ACTION] Defaulting to BUY (no action keyword found)")
        ...
        if sl_match:
            ...
            # DEFAULT SL: If no SL found and we have entry price, set SL to 5% below entry
            if result.get('entry_price'):
                default_sl = result['entry_price'] * 0.95
                result['stop_loss'] = round(default_sl, 2)
                result['sl_auto_added'] = True
                self.logger.info(f"[SL AUTO] No SL found, using 5% below entry: {result['stop_loss']}")
    ...
    def _parse_options(self, message):
        ...
        # Entry price
        entry_patterns = [
            r'(?:ABOVE|NEAR|LEVEL)\s*[-:\.]*\s*(\d+\.?\d*)',  # "ABOVE 95"
            r'(\d+\.?\d*)\s*ABOVE',  # "225 ABOVE" (NEW!)
            r'ENTRY\s*[-:]*\s*(\d+\.?\d*)',
            r'CMP\s*[-:]*\s*(\d+\.?\d*)',
