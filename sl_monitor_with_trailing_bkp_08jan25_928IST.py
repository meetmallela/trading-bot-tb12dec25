...
class EnhancedSLMonitor:
    ...
    def calculate_trailing_sl(self, position, current_ltp, current_sl_trigger, signal_data=None):
        ...
        if quantity > 0:
            ...
            if signal_data and signal_data.get('stop_loss'):
                # Signal-based trailing logic
                # After some time, move SL to AT LEAST 95% of entry
                
                sl_95_entry = int(avg_price * 0.95)
                threshold_price = avg_price * 1.05
                
                # If current SL is below 95% of entry, move it up
                if current_sl_trigger and current_sl_trigger < sl_95_entry:
                    logging.info(f"[TRANSITION] Moving SL from {current_sl_trigger} to 95% of entry: {sl_95_entry}")
                    return sl_95_entry, "TRANSITION"
                
                # Check if ready to trail (LTP above 105% of entry)
                if current_ltp >= threshold_price:
                    # Trail to 95% of LTP
                    new_sl = int(current_ltp * 0.95)
    ...
    def monitor_open_positions(self):
        ...
        try:
                ...
                if existing_sl:
                    ...
                    if (sl_type in ["TRAILING", "TRANSITION"]) and new_sl > current_sl_trigger:
                        ...
                        if sl_type == "TRAILING":
                            ...
                            logging.info(f"   [UPDATE] Moving SL from {current_sl_trigger} to {new_sl}")
            ...
            # IMPORTANT: Check for positions that were closed (SL triggered)
            # Compare currently open positions with previously protected positions
